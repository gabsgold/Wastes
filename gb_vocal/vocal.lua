...
-- Réglage de la portée via deux touches (exemple : + et -)
Citizen.CreateThread(function()
	while true do
		Citizen.Wait(0)
		if IsControlJustPressed(1, Keys["PAGEUP"]) then -- Touche +
		  x = x + y
		  if x >= 20 then -- Portée maximale
		    x = 20.0 -- Portée maximale
		  end
		  NetworkSetTalkerProximity(x)
		  Texte("Portée de la voix : ".. x .."m", 5000)
		end
		if IsControlJustPressed(1, Keys["PAGEDOWN"]) then -- Touche -
		  x = x - y
		  if x <= 2 then -- Portée minimale
		    x = 2.0 -- Portée minimale
		  end
		  NetworkSetTalkerProximity(x)
		  Texte("Portée de la voix : ".. x .."m", 5000)
		end
...
-- Réglage de la portée via une seule touche
Citizen.CreateThread(function()
	while true do
		Citizen.Wait(0)
		if IsControlJustPressed(1, Keys["H"]) then
		  x = x + y
		  if x >= 22 then -- Portée maximale
		    x = 2.0 -- Si portée maximale atteinte alors portée égale à 2m
		  end
		  NetworkSetTalkerProximity(x)
		  Texte("Portée de la voix : ".. x .."m", 5000)
		end
